...
function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage('Reset token berhasil dibuat. Gunakan token di bawah ini untuk mengatur ulang kata sandi.');
      setError('');
    } catch (err) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-100 via-blue-50 to-gray-200 py-12 px-4 sm:px-6 lg:px-8">
      <motion.div variants={cardVariants} initial="hidden" animate="visible" className="max-w-md w-full bg-white/80 backdrop-blur-lg rounded-2xl shadow-2xl p-8 space-y-8 border border-gray-100/50">
        <div className="text-center">
          ...
          >
            Masukkan email Anda untuk mendapatkan token reset.
          </motion.p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-6">
          {message && (
            <motion.p variants={messageVariants} initial="hidden" animate="visible" className="text-center text-sm text-secondary bg-green-100/80 p-3 rounded-md">
              {message}
              {resetToken && (
